...
public class DlgInputStok extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        BtnPrint1.setText("Cetak Opname");
...
//                Valid.MyReportqry("rptInputStokOpname.jasper","report","::[ Data Persediaan Stok Ruangan ]::","select * from temporary where temporary.temp37='"+akses.getalamatip()+"' order by temporary.no",param);
                ...
                Valid.MyReportqry("rptInputStokOpname.jasper","report","::[ Data Persediaan Stok Ruangan ]::","select no, temp1, temp2, temp3, temp4, temp5, temp6, temp7, temp8, temp9, temp10, temp11, temp12, temp13, temp14, (temp5 * temp6) as total from temporary where temporary.temp37='"+akses.getalamatip()+"' order by temporary.no",param);            
            }
            this.setCursor(Cursor.getDefaultCursor());
        }
    ...
    private void BtnPrint1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint1ActionPerformed
        if(nmgudang.getText().trim().equals("")||kdgudang.getText().trim().equals("")){
            ...
            param.put("logo",Sequel.cariGambar("select setting.logo from setting"));
            param.put("bangsal",nmgudang.getText());
            param.put("tgl1",Tgl.getDate());
//            param.put("tgl2",DTPCari2.getDate());
//            Valid.MyReportqry("rptInputStokOpnameAlt.jasper","report","::[ Transaksi Mutasi Obat/Alkes/BHP ]::","SELECT opname.kode_brng, databarang.nama_brng, databarang.kode_sat,opname.h_beli, opname.real, (opname.h_beli*opname.real) as total, DATE_FORMAT(opname.tanggal,'%d-%m-%Y') as tanggal FROM opname INNER JOIN databarang ON opname.kode_brng=databarang.kode_brng where opname.tanggal between '"+Valid.SetTgl(DTPCari1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(DTPCari2.getSelectedItem()+"")+"' order by opname.tanggal asc",param);          
            Valid.MyReportqry("rptInputStokOpnameAlt.jasper","report","::[ Transaksi Mutasi Obat/Alkes/BHP ]::","SELECT opname.kode_brng, databarang.nama_brng, databarang.kode_sat,opname.h_beli, opname.real, (opname.h_beli*opname.real) as total, DATE_FORMAT(opname.tanggal,'%d-%m-%Y') as tanggal FROM opname INNER JOIN databarang ON opname.kode_brng=databarang.kode_brng where opname.tanggal='"+Valid.SetTgl(Tgl.getSelectedItem()+"")+"'",param);          
