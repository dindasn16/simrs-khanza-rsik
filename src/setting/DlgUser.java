...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[I]ICD 10","[I]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[L]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[K]Mapping Vaksin Satu Sehat","[E]Suplier Dapur","[X]Hapus Data Salah","[X]Hapus Data Lab"
        };
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "hapus_data_salah='"+tbUser.getValueAt(i,913).toString()+"'," +
                    "hapus_data_lab='"+tbUser.getValueAt(i,914).toString()+"'");
            }            
            tampil();
            emptTeks();
        }
                            ...
                            try {
                                ...
                                if(i!= -1){
                                    Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                                        ...
                                        "hapus_data_salah='"+tbUser.getValueAt(barisdicopy,913).toString()+"',"+
                                        "hapus_data_lab='"+tbUser.getValueAt(barisdicopy,914).toString()+"'");
                                }    
                                userdicopy="";
                                copyhakakses="";
                                barisdicopy=-1;
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.signout_sebelum_menutup_luka,user.dapur_barang,user.dapur_opname,user.satu_sehat_mapping_vaksin,user.dapur_suplier,user.hapus_data_salah,user.hapus_data_lab from user order by AES_DECRYPT(user.id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
