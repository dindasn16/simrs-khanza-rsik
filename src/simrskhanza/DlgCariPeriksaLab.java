            ...
            if(Sequel.cariRegistrasi(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString())>0){
                ...
                if (reply == JOptionPane.YES_OPTION) {
                    try{
                        ...
                        if(sukses==true){
                            Sequel.Commit();
                            JOptionPane.showMessageDialog(null,"Data Periksa Lab Berhasil Terhapus !!!");                            
                            tampil();
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {   
                ps4=koneksi.prepareStatement(
                    ...
                    "pasien.no_ktp,pasien.pekerjaan,(pasien.nip)as nrp,reg_periksa.kd_pj,"+
                ...
                try {
                    ...
                    while(rs.next()){
                        ...
                        param.put("nrp",rs.getString("nrp"));                        
                        ...
                        param.put("perusahaan",Sequel.cariIsi("select perusahaan_pasien.nama_perusahaan from perusahaan_pasien where perusahaan_pasien.kode_perusahaan=?",Sequel.cariIsi("select pasien.perusahaan_pasien from pasien where pasien.no_rkm_medis=?",rs.getString("no_rkm_medis"))));
                        param.put("carabayar",Sequel.cariIsi("select penjab.png_jawab from penjab where kd_pj=?",Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.kd_pj=?",rs.getString("kd_pj"))));                        
...
//        BtnHapus.setEnabled(akses.getperiksa_lab());
        BtnHapus.setEnabled(akses.gethapus_data_lab());
