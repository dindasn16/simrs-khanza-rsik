...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            timeout_sebelum_insisi=false,signout_sebelum_menutup_luka=false,dapur_barang=false,dapur_opname=false,satu_sehat_mapping_vaksin=false,dapur_suplier=false,hapus_data_salah=false,hapus_data_lab=false;
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.signout_sebelum_menutup_luka,user.dapur_barang,user.dapur_opname,user.satu_sehat_mapping_vaksin,user.dapur_suplier,user.hapus_data_salah,user.hapus_data_lab from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    dapur_barang=rs.getBoolean("dapur_barang");dapur_opname=rs.getBoolean("dapur_opname");satu_sehat_mapping_vaksin=rs.getBoolean("satu_sehat_mapping_vaksin");dapur_suplier=rs.getBoolean("dapur_suplier");hapus_data_salah=rs.getBoolean("hapus_data_salah");hapus_data_lab=rs.getBoolean("hapus_data_lab");
                    setTampil();
                }       
    ...
    private void setTampil(){
        ...
        if("[X]Hapus Data Salah".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[X]Hapus Data Salah",hapus_data_salah});
        } 
        
        if("[X]Hapus Data Lab".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[X]Hapus Data Lab",hapus_data_lab});
        }         
    }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[X]Hapus Data Salah".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","hapus_data_salah='"+tbUser.getValueAt(i,2).toString()+"'");
            }  
            
            if("[X]Hapus Data Lab".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","hapus_data_lab='"+tbUser.getValueAt(i,2).toString()+"'");
            }             
        }
